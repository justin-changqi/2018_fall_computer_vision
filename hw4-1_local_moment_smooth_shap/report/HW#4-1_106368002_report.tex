	...
	\begin{reportsection}{-1cm}{-1cm}
      ...
      \item 
        ...
        Discussion, 10\%)
				...
				When local mask mean $m_{m}$ is less than full image mean $m_{G}$ times a constant $k_{0}$ and 
				local standard deviation $\sigma_{m}$ is between full image standard deviation $\sigma_{G}$ times
				constant $k_1$ and $k_2$, then we enhance pixel value $f(x, y)$ by constant $E$. In this image, 
				we are trying to enhance the dark part so I set $k_{0}=0.4$. Set standard deviation range by 
				experiment. The perform range for standard deviation is $0.01\sigma_{G} \leq \sigma_{m} \leq 
				0.4\sigma_{G}$. E constant set as 2.  
				\begin{equation}
					g(x, y) = 
					\begin{cases} 
    ...
    \section*{Problem 2 Image Smoothing (60\%)}
      ...
      \item 
				...
				\begin{figure}[htbp]
					...
					\caption{LR mask and Figure~\ref{fig:sheep-masking} mask logic operation result}
					\label{fig:filter-masking}
				\end{figure}
        % \blindtext[1]
    \end{enumerate}
