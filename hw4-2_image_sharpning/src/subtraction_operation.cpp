...
cv::Mat sharpingImg2(cv::Mat &src, cv::Mat &src_lpf, double c)
{
  cv::Mat out_img(src.rows, src.cols, CV_32SC1);
  for (int i = 0; i < src.rows; i++)
  {
    for (int j = 0; j < src.cols; j++)
    {
      int px = 0;
      px = src.at<uint8_t>(i, j) + c*(src.at<uint8_t>(i, j) - src_lpf.at<uint8_t>(i, j));
      px = std::max(0, px);
      // px = std::min(255, px);
      out_img.at<int>(i, j) = px;
    }
  }
  return out_img;
}
...
cv::Mat NormalizeInt(cv::Mat &src)
{
  cv::Mat nor_mat(src.rows, src.cols, CV_8UC1);
  std::vector<int> px_list;
  for (int i = 0; i < src.rows; i++)
...
int main(int argc, char **argv)
{
  ...
  // sharped image
  cv::Mat sharped2_03 = sharpingImg2(src, box_img, 0.3);
  cv::Mat sharped2_05 = sharpingImg2(src, box_img, 0.5);
  cv::Mat sharped2_08 = sharpingImg2(src, box_img, 0.8);
  cv::Mat sharped2_1 = sharpingImg2(src, box_img, 1);
  cv::Mat sharped2_n_08 = sharpingImg2(src_n, box_n_img, 0.8);
  cv::Mat sharped2_03_nor = NormalizeInt(sharped2_03);
  cv::Mat sharped2_05_nor = NormalizeInt(sharped2_05);
  cv::Mat sharped2_08_nor = NormalizeInt(sharped2_08);
  cv::Mat sharped2_1_nor = NormalizeInt(sharped2_1);
  cv::Mat sharped2_n_08_nor = NormalizeInt(sharped2_n_08);
  showImage("src", src);
  showImage("sharped 0.3", sharped2_03_nor);
  showImage("sharped 0.5", sharped2_05_nor);
  showImage("sharped 0.8", sharped2_08_nor);
  showImage("sharped 1", sharped2_1_nor);
  showImage("sharped noise", sharped2_n_08_nor);
  ...
  saveImage(sharped2_03_nor, "../result_img/", "sharped2_03");
  saveImage(sharped2_05_nor, "../result_img/", "sharped2_05");
  saveImage(sharped2_08_nor, "../result_img/", "sharped2_08");
  saveImage(sharped2_1_nor, "../result_img/", "sharped2_1");
  saveImage(sharped2_n_08_nor, "../result_img/", "sharped2_n_08");
  cv::waitKey(0);
  return 0;
}